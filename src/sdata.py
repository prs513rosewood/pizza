...
class sdata:
    ...
    def __init__(self, *list):
        ...
        if len(list) == 1:
            raise Exception("surf ID and surf file required")
        if len(list) > 1:
            self.read(list[0], *list[1:])
    ...
    def read(self, id, *list):
            ...
            while 1:
                ...
                if "points" in line:
                    ...
                    for i in range(nlines):
                        list = f.readline().split()
                        lines.append(
                            [int(value)-npoints_prev-1 for value in list[1:]])
        ...
        if self.dim == 2:
            print("surf %s with %d points, %d lines" %
                  (id, npoints, nlines))
        if self.dim == 3:
            print("surf %s with %d points, %d triangles" %
        ...
        box = bbox(points)
        surf.center = [0.5*(box[0]+box[3]), 0.5 *
                       (box[1]+box[4]), 0.5*(box[2]+box[5])]
    ...
    def write(self, file, *list):
        if not len(list):
            vlist = list(range(len(self.surfs)))
        else:
            vlist = []
            for id in list:
        ...
        if self.dim == 3:
            print(len(triangles), "triangles", file=fp)
        print(file=fp)
        print("Points\n", file=fp)
        if self.dim == 2:
    ...
    def grid(self, *args):
        if self.dim == 0:
            raise Exception("dimension must be defined for grid")
    ...
    def gridfile(self, *args):
        if self.dim == 0:
            raise Exception("dimension must be defined for gridfile")
        ...
        if self.dim == 2:
            ...
            for surf in self.surfs:
                ...
                for segment in segments:
                    id += 1
                    lines.append(
                        [id, itype] + points[segment[0]] + points[segment[1]])
        ...
        if self.gridflag and self.dim == 2:
            for idparent in self.idparents:
                ...
                for i in range(nx+1):
                    ...
                    lines.append([id+i, itype+nlevel+1] +
                                 [x, ylo, 0.0, x, yhi, 0.0])
                id += nx+1
                for i in range(ny+1):
        ...
        if self.gridflag and self.dim == 3:
            for idparent in self.idparents:
                ...
                if not box:
                    ...
                    iz = (idchild-1) / (nx*ny)
                    box = (xlo + float(ix)*(xhi-xlo)/nx, xlo + float(ix+1)*(xhi-xlo)/nx,
                           ylo + float(iy)*(yhi-ylo)/ny, ylo +
                           float(iy+1)*(yhi-ylo)/ny,
