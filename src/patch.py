...
class patch:
    ...
    def write3d(self, file):
        ...
        while self.molecules:
            ...
            if self.style == "molecular":
                ...
                for atom in molecule[0]:
                    ...
                    x, y, z, ix, iy, iz = self.pbc(x, y, z, ix, iy, iz)
                    atoms.append(
                        [idatom, atom[0], 1.0, 1.0, x, y, z, ix, iy, iz])
                    mols.append((idatom, idmol))
                    maxtypes = max(maxtypes, atom[0])
        ...
        if tris:
            d.headers["triangles"] = len(tris)

        if tris:
            print("TRIS", len(tris), d.headers["triangles"])
            ...
            if self.style == "molecular":
                ...
                for i, atom in enumerate(molecule[0]):
                    ...
                    if not segments:
                        ...
                        lineflag = 1
                    atoms.append(
                        [idatom, idmol, atom[0], lineflag, mass, x, y, z, ix, iy, iz])
                    mols.append((idatom, idmol))
                    maxtypes = max(maxtypes, atom[0])
                ...
                if not subs:
                    ...
                    bodyflag = 1
                atoms.append([idatom, atom[0], bodyflag,
                              mass, x, y, z, ix, iy, iz])
                mols.append((idatom, idmol))
                maxtypes = max(maxtypes, atom[0])
    ...
    def hex(self, *params):
        ...
        for i in range(m4):
            if i == 0:
                ...
                bonds.append([1, n+m1+m2+m3+i-1, n+m1+m2+m3+i])
        for i in range(m5):
            if i == 0:
                bonds.append([1, 5, n+m1+m2+m3+m4])
            else:
                bonds.append([1, n+m1+m2+m3+m4+i-1, n+m1+m2+m3+m4+i])
        for i in range(m6):
            if i == 0:
                bonds.append([1, 6, n+m1+m2+m3+m4+m5])
            else:
                bonds.append([1, n+m1+m2+m3+m4+m5+i-1, n+m1+m2+m3+m4+m5+i])
